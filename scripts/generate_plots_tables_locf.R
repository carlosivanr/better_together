...
# MBI EE Sum -------------------------------------------------------------------
# Run the function
mbi_ee_list <- disp_stats_plot("mbi_ee_sum", "locf")

# Display plot
mbi_ee_list[[4]]

# Save plot
ggsave(here("figures", "locf", "mbi_ee_sum.png"), 
       height = 4, 
       width = 5.5, 
       device='png', 
       dpi=600)
...
# MBI DP Sum -------------------------------------------------------------------
# Run the function
mbi_dp_list <- disp_stats_plot("mbi_dp_sum", "locf")

# Display plot
mbi_dp_list[[4]]

# Save plot
ggsave(here("figures", "locf", "mbi_dp_sum.png"), 
       height = 4, 
       width = 5.5, 
       device='png', 
       dpi=600)
...
# MBI PA Sum -------------------------------------------------------------------
# Run the function
mbi_pa_list <- disp_stats_plot("mbi_pa_sum", "locf")

# Display plot
mbi_pa_list[[4]]

# Save plot
ggsave(here("figures", "locf", "mbi_pa_sum.png"), 
       height = 4, 
       width = 5.5, 
       device='png', 
       dpi=600)
...
# Run the function
misshp_list <- disp_stats_plot("misshp_tot", "locf")

# Display plot
misshp_list[[4]]

# Save plot
ggsave(here("figures", "locf", "misshp.png"), 
       height = 4, 
       width = 5.5, 
       device='png', 
       dpi=600)
...
yissum_list <- disp_stats_plot("yissum", "locf")

# Display plot
yissum_list[[4]]

# Save plot
ggsave(here("figures", "locf", "yissum.png"), 
       height = 4, 
       width = 5.5, 
       device='png', 
       dpi=600)
...
ls3_list <- disp_stats_plot("ls3_tot", "locf")

# Display plot
ls3_list[[4]]

# Save plot
ggsave(here("figures", "locf", "ls3.png"), 
       height = 4, 
       width = 5.5, 
       device='png', 
       dpi=600)
...
scssf_list <- disp_stats_plot("scssf_sum_tot", "locf")

# Display plot
scssf_list[[4]]

# Save plot
ggsave(here("figures", "locf", "scssf.png"), 
       height = 4, 
       width = 5.5, 
       device='png', 
       dpi=600)
...
# Flourishing Index ------------------------------------------------------------
fi_list <- disp_stats_plot("fi_tot", "locf")

# Display plot
fi_list[[4]]

# Save plot
ggsave(here("figures", "locf", "fi.png"), 
       height = 4, 
       width = 5.5, 
       device='png', 
       dpi=600)
...
sfi_list <- disp_stats_plot("sfi_tot", "locf")

# Display plot
sfi_list[[4]]

# Save plot
ggsave(here("figures", "locf", "sfi.png"), 
       height = 4, 
       width = 5.5, 
       device='png', 
       dpi=600)
...
gt::gtsave(long_table, "estimate_ci_p.rtf", path = here("tables", "locf"))
...
for (i in dependent_vars){
  ...
  # Get the ids of those who did not have a pre test score, they will not be 
  # included in the participant n count, they will not have a pre nor a post
  # score in the data since the locf algorithm will not carry last observation 
  # backward
  no_pretest_ids <- 
    data %>% 
    select(identifier, !!rlang::sym(i), redcap_event_name.factor) %>% 
    filter(is.na(!!rlang::sym(i)), redcap_event_name.factor == "Pre") %>% 
    pull(identifier)
  
  # Who did not have a pre test score, but had a post test score
  # In other words, who had post test only.
  data %>% 
    select(identifier, !!rlang::sym(i), redcap_event_name.factor) %>% 
    filter(identifier %in% no_pretest_ids,
           !is.na(!!rlang::sym(i))) %>%
    select(identifier)
...
write_csv(final_table, na = "", file = here("tables", "locf", "mixed_mod_results_table.csv"))
